...
const GTFS = join(ROOT, 'data/gtfs-pt');
...
const osm = { elements: [] };
{
  // the road network is eight Overpass tiles (see build.mjs ROAD_TILES) — merged
  // here the same way, ways deduped, so street names cover the whole province
  const seen = new Set();
  for (let r = 0; r < 2; r++) for (let c = 0; c < 4; c++) {
    const part = JSON.parse(readFileSync(join(ROOT, `data/osm/istanbul-r${r}c${c}.json`), 'utf8'));
