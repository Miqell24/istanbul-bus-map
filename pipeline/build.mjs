...
// Istanbul: TWO İBB feeds, repaired by pipeline/normalize.mjs first.
//   data/gtfs-iett — 796 bus lines, NO shapes: stop sequences are the only
//                    geometry, so every run is pseudo-matched (as in Rybnik).
//   data/gtfs-pt   — rail with shapes, split by route_type: 0 trams (T1/T3/T4),
//                    1 metro (M1A…M9) and Marmaray (agency TCDD), 7 funiculars.
// The metro and Marmaray keys trigger the engine's metro treatment (wide
// ribbon, station discs, always-on names).
// Usage: node pipeline/build.mjs [--all | lines...] [--tram all|T1,M2]
...
// Lines that get the metro treatment: wide ribbon, station discs, names on at
// every zoom. Marmaray is a commuter railway, but it runs metro-style through
// the city and the Bosphorus tunnel, so it is drawn the same way.
const METRO_KEY = /^(M[0-9]|Marmaray)/i;
...
// Rail line colors. M1A/M1B, M2, M3, M4, T4, T5 and the funiculars carry a
// `colour` tag on their OSM route_master relation — those hexes are used as
// published. OSM records nothing for M5–M9, T1, T3 and Marmaray, and the feed's
// route_color column is empty for every line, so those follow the operator's
// printed scheme as closely as it can be pinned down; they are marked below.
const RAIL_COLORS = {
  M1A: '#E32726', M1B: '#E32726', // OSM
  M2: '#009A4C', M2A: '#009A4C', // OSM (M2A is the Seyrantepe branch)
  M3: '#00A8E1', M3A: '#00A8E1', // OSM
  M4: '#E91E76', // OSM
  M5: '#7B3F9E', M6: '#A3843C', M7: '#B0208C', M8: '#00A19A', M9: '#F2B705', // scheme
  T1: '#005EB8', T3: '#8C5A2B', // scheme
  T4: '#E87400', // OSM ("orange")
  F1: '#7A745A', F2: '#7A745A', F3: '#7A745A', // OSM
  Marmaray: '#00539F', Marmaray1: '#00539F', Marmaray2: '#00539F', // scheme
};
const TCDD = '4'; // agency_id of Marmaray's operator inside the rail feed

// The road network is fetched as eight Overpass tiles (see download.sh) — the
// province is far too large for one call — and merged at load time.
const ROAD_TILES = [];
for (const r of [0, 1]) for (const c of [0, 1, 2, 3]) ROAD_TILES.push(`data/osm/istanbul-r${r}c${c}.json`);

const MODES = [{
  mode: 'bus', label: 'buses', osmFile: ROAD_TILES,
  graphMode: 'road', color: '#0059a9', colorDark: '#00294f',
  all: busAll, lines: busList.length ? busList : (busAll ? [] : ['34G']),
  feeds: [
...
// its own kind of track and they must NOT share a graph: a metro shape often
// passes closer to the tram tracks above its tunnel than to the subway axis,
// and Marmaray runs on mainline rail that the metro must never borrow.
// railKeep filters the OSM ways per cfg before the graph is built.
const tramAll = tramLines.length === 1 && tramLines[0] === 'all';
const railSel = (pred) => (tramAll ? [] : tramLines.filter(pred));
...
const railCfg = (label, railKeep, routeTypes, pick, sel) => ({
  ...
  color: '#d6212b', colorDark: '#7c1116', palette: RAIL_COLORS,
  all: tramAll, lines: sel,
  feeds: [{ ...RAIL, routeTypes, mapKey: (sn, r) => (pick(sn, r) ? sn : null) }],
});
{
  const sel = railSel((l) => !isMetroKey(l) && !isFuniKey(l) && !isMarmaray(l));
  if (tramAll || sel.length) MODES.push(railCfg('trams', ['tram', 'light_rail'], ['0'], () => true, sel));
}
{
  const sel = railSel(isMetroKey);
  if (tramAll || sel.length) {
    MODES.push(railCfg('metro', ['subway', 'light_rail'], ['1'], (sn, r) => r.agency_id !== TCDD, sel));
  }
}
{
  const sel = railSel(isMarmaray);
  if (tramAll || sel.length) {
    MODES.push(railCfg('Marmaray', ['rail', 'light_rail'], ['1'], (sn, r) => r.agency_id === TCDD, sel));
  }
}
{
  // NOT part of `--tram all`: the feed gives only the two end points of each
  // funicular and OSM has mere fragments of the tunnels (F1 Taksim–Kabataş is a
  // 120 m way for a 600 m line), so every run fails to match. Build them by
  // name (`--tram F1`) if the OSM data ever improves.
  const sel = railSel(isFuniKey);
  if (sel.length) MODES.push(railCfg('funiculars', ['funicular', 'rail'], ['7'], () => true, sel));
}
...
async function processMode(cfg) {
  ...
  const isMetroMix = (lines) => cfg.mode === 'tram' && lines.length > 1 && lines.every((l) => METRO_KEY.test(l));
  const colorOf = (lines) => {
  ...
  // Metrobüs: İETT's 34-family runs on the segregated busway between
  // Söğütlüçeşme and Beylikdüzü, not in traffic, so it takes the engine's amber
  // "own right of way" category — the same slot the GZM map gives its
  // metrolines. Every 34* line of the feed belongs to that corridor.
  cfg.mlineSet = new Set();
  // cfg.palette seeds the per-line colors for feeds that ship no route_color
  cfg.lineColors = { ...(cfg.palette || {}) };
  cfg.lineColorsDark = {};
  for (const [k, v] of Object.entries(cfg.lineColors)) cfg.lineColorsDark[k] = darken(v, 0.45);
  ...
  for (const feed of cfg.feeds) {
    ...
    for (const r of await readCsv(join(fdir, 'routes.txt'))) {
      ...
      if (cfg.mode === 'bus' && /^34[A-ZÜÇŞĞİÖ]*$/.test(key)) {
    ...
    // TWO passes over stop_times, not one. The old single pass held the stops of
    // every candidate trip in memory just to find the longest one — fine for a
    // 3 MB feed, fatal for İETT's 6.2 M-row, 150 MB table (up to 80 candidates ×
    // ~1 600 line/direction pairs). Pass 1 only counts rows per trip, pass 2
    // reads the stops of the ONE winning trip per line/direction.
    ...
    const wanted = new Map(); // winning trip id → its rep
    for (const r of feedReps) {
      let bestTrip = null, bestLen = -1;
      for (const id of r.candTrips) {
        const n = stopCount.get(id) ?? 0;
        if (n > bestLen) { bestLen = n; bestTrip = id; }
      }
      r.stopSeq = [];
      if (bestTrip) wanted.set(bestTrip, r);
    }
    stopCount.clear();
    for await (const st of iterCsv(join(fdir, 'stop_times.txt'))) {
      const r = wanted.get(st.trip_id);
      if (!r) continue;
      r.stopSeq.push({ seq: Number(st.stop_sequence), stopId: feed.tag + ':' + st.stop_id });
    }
    for (const r of feedReps) r.stopSeq.sort((a, b) => a.seq - b.seq);
  ...
  // osmFile may be a list: the road network arrives as eight Overpass tiles,
  // and ways that straddle a tile border come back in both, so ids are deduped.
  const osm = { elements: [] };
  {
    const seen = new Set();
    for (const f of [cfg.osmFile].flat()) {
    ...
    if ([cfg.osmFile].flat().length > 1) log(`OSM: ${osm.elements.length} ways from ${[cfg.osmFile].flat().length} tiles`);
  }
  ...
  for (const r of reps) {
    ...
    if (r.pseudo) {
      ...
      if (cfg.mode === 'tram' && METRO_KEY.test(r.line)) {
        // wider net than the sibling cities: the 2023-era İBB shapes are street
        // sketches that drift up to ~200 m off the OSM tunnel axis (M3 lost a
        // third of its observations at 120 m and tore twice near Başakşehir)
        opts = { sigma: 22, radii: [80, 220], maxCand: 16, perWay: 3, gapMin: GAP_MIN };
      }
    }
  ...
  if (cfg.mode === 'tram') {
    const isMetroEntry = (e) => [...e.lines].every((l) => METRO_KEY.test(l));
  ...
  for (const e of stopAgg.values()) {
    ...
    const isMetroStop = cfg.mode === 'tram' && [...e.lines].every((l) => METRO_KEY.test(l));
    ...
    if (p.terminus && p.label && p.badgeLines && p.badgeLines.length) {
      badgeAnchors.push({
        ...
        lines: p.badgeLines.map((line) => ({
          ...
          ...(p.mode === 'tram' && METRO_KEY.test(line) ? { metro: 1 } : {}),
        })),
      });
    }
    delete p.termArr;
  ...
  const runFlags = (arr) => {
    ...
    if (cfg.mode === 'tram' && arr.every((l) => METRO_KEY.test(l))) flags.metro = 1;
    if (cfg.mlineSet && cfg.mlineSet.size) {
      const n = arr.filter((l) => cfg.mlineSet.has(l)).length;
